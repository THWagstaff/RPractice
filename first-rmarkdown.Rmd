...
covid_state_data %>% 
  filter(state %in% c("California","Florida", "Texas", "Illinois", "New York")) %>% 
  ggplot(aes(x = date, y = daily_new_cases)) +
  geom_line() +
  facet_wrap(~state, nrow = 5, ncol = 2)
```
...
m <- leaflet() %>%
  ...
  addMarkers(lng=43.818301, lat=-111.782565, popup="The birthplace of R")
m  # Print the map
```
